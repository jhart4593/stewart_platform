...
//pixy2 cam
float origin[2] = {144, 108};  // X and Y co-ords of the origin
float r_platform = 150;          // the distance from the center of the platform to the corner of the platform seen from the pixy2 cam
...
float kp_p = 6e-4;      // proportional constant position
float kd_p = 0.56;      // derivative constant position
float kp_o = 0.0013;      // proportional constant orientation
float kd_o = 1.1;      // derivative constant orientation
...
float pos_max = 0.5; // max x/y position of platform origin allowed
float or_max = PI/4; // max roll/pitch of platform origin allowed
...
void setup() {
  ...
  ball[0] = 280;
  ball[1] = 50;
  float ball_pos_prev[2] = {300,60};
  err_prev[0] = origin[0] - ball_pos_prev[0];
  err_prev[1] = ball_pos_prev[1] - origin[1];
  dt = 100;
  ...
  //   float x_add = i*(-5);
  //   float y_add = i*(-5);
  //   float ball_pos_prev[2] = {300+x_add,60+y_add};
  //   err_prev[0] = origin[0] - ball_pos_prev[0];
  //   err_prev[1] = ball_pos_prev[1] - origin[1];



  //   ball[0] = ball_pos_prev[0];
  //   ball[1] = ball_pos_prev[1];
  // }
}
...
void IK(float pose_des[]){  //calculates servo arm angles given desired pose of the platform 
  ...
  // BLA::Matrix<2,2> t1 = {1,2,3,4};
  // BLA::Matrix<2,2> t2 = {5,7,6,8};
  // BLA::Matrix<2,2> t3 = t1*t2;
  // Serial.println(t3(1,1));
  // Serial.println(t3);

  //calculate effective leg lengths
  //translation vector of platform frame wrt base frame
  BLA::Matrix<3,1> T = {pose_des[0],pose_des[1],pose_des[2]+h_0};
  ...
  Serial.println((String)"Alpha_cmd: [" + alpha_cmd[0]+", "+alpha_cmd[1]+", "+alpha_cmd[2]+", "+alpha_cmd[3]+", "+alpha_cmd[4]+", "+alpha_cmd[5]+"]");
}
...
void controller(){// calculates the PD controller values and moves the servos
  ...
  if (r_ball > r_platform) {
    ...
    //moveServos(20, 20);
    Serial.println("r_ball greater than r_platform");
  }
    ...
    //cmd[2] = (err[0] * kp_o) + (d_err[0] * kd_o);  // calculates commanded platform roll
    //cmd[3] = (err[1] * kp_o) + (d_err[1] * kd_o);  // calculates commanded platform pitch
    ...
    Serial.println((String)"pos_cmd: [" + cmd[0]+", "+cmd[1]+"]");
